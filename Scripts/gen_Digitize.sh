...
usage() { echo "Usage: $0 [ --primary primary physics process name ]
  ...
  [ --steps (opt)  If specified, use this list of input dts.*.art files instead of the one generated by SAM ]
...
OUTDESC=${PRIMARY}${DIGITYPE}
if [[ "${DIGITYPE}" == "Extracted" || "${DIGITYPE}" == "NoField" ]]; then
  if [[ "${PRIMARY}" != *"${DIGITYPE}"* ]]; then
    echo "Error: PRIMARY ${PRIMARY} doesn't match digitization type ${DIGITYPE}"
    exit_abnormal
  else
  ...
  if [[ "${PRIMARY}" == *"Extracted"* || "${PRIMARY}" == *"NoField"* ]]; then
    echo "Error: PRIMARY ${PRIMARY} incompatible with digitization type ${DIGITYPE}"
    exit_abnormal
  fi
fi
...
if [[ -n $DSTEPS ]];
then
  echo "Using user-provided input list of primary DetectorStep files $DSTEPS"
...
if [[ "${DIGITYPE}" == "Extracted" || "${DIGITYPE}" == "NoField" ]]; then
  echo outputs.TrkOutput.fileName: \"dig.owner.${OUTDESC}Trk.version.sequencer.art\" >> digitize.fcl
  echo outputs.CaloOutput.fileName: \"dig.owner.${OUTDESC}Calo.version.sequencer.art\" >> digitize.fcl
  echo outputs.UntriggeredOutput.fileName: \"dig.owner.${OUTDESC}Untriggered.version.sequencer.art\" >> digitize.fcl
else
  # keep all streams
  echo outputs.SignalOutput.fileName: \"dig.owner.${OUTDESC}Signal.version.sequencer.art\" >> digitize.fcl
  echo outputs.DiagOutput.fileName: \"dig.owner.${OUTDESC}Diag.version.sequencer.art\" >> digitize.fcl
  echo outputs.TrkOutput.fileName: \"dig.owner.${OUTDESC}Trk.version.sequencer.art\" >> digitize.fcl
  echo outputs.CaloOutput.fileName: \"dig.owner.${OUTDESC}Calo.version.sequencer.art\" >> digitize.fcl
  echo outputs.UntriggeredOutput.fileName: \"dig.owner.${OUTDESC}Untriggered.version.sequencer.art\" >> digitize.fcl
fi
...
generate_fcl --dsconf="${OUTCONF}" --dsowner=${OWNER} --description="${OUTDESC}" --embed digitize.fcl \
