## 🧪 Bash Script Documentation: Introducing New Physics `Stage4_signal.sh`

### **1. Overview**

The `Stage4_signal.sh` script is designed for **signal injection** to create multiple mixed (signal + background) "pseudo-experiment" datasets. This allows physicists to test analysis techniques under specific signal hypotheses without running full-scale, expensive simulations for every $R_{\mu e}$ rate.
...
```bash
Stage4_signal.sh --known <MDS_TAG> --signal <SIGNAL_NAME> --rate <RATE> [OPTIONS]
```
...
1.  **Retrieve Stage 2 Config:** The script reads the configuration file (`${KNOWN}.txt`) generated by Stage 1-3 to extract the original `GEN_LIVETIME` and `BB` (Beam Batch) mode of the background sample.
...
This utility script is designed to run after `Stage4_signal.sh`. It takes the long list of randomly chosen individual background Ntuples and the newly generated signal Ntuple for a single pseudo-experiment (PE) and merges them into a smaller, more manageable set for final analysis.
...
| **Input** | A list of Ntuple files (`filenames_ChosenMixed_$i`) generated in Stage 4, and the configuration file (`${KNOWN}.txt`). |
| **Tool Used** | `hadd` (ROOT Histo Adder) – a utility specifically designed for merging ROOT files while preserving their data structures.  |
